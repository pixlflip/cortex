...
function Shell() {
  const auth = useAuth(); const [mobile, setMobile] = useState(false)
...
function VaultPage() {
  ...
  useEffect(() => { const handle = setTimeout(async () => { if (!query.trim() || !selectedVault) { setResults([]); return } setSearching(true); try { const data = await api<{ results: any[] }>(`/vaults/${selectedVault}/search?q=${encodeURIComponent(query)}`); setResults(data.results) } finally { setSearching(false) } }, 180); return () => clearTimeout(handle) }, [query, selectedVault])
  ...
  return <div className="vault-page">
    ...
    <div className="vault-layout">
      <aside className="vault-tree panel"><div className="search-box"><Search /> <input placeholder="Search this vault…" value={query} onChange={e => setQuery(e.target.value)} /><kbd>⌘K</kbd></div>
        {query && <div className="search-popover">{searching && <small>Searching…</small>}{results.map(result => <button key={`${result.path}-${result.line}`} onClick={() => { open(result.path); setQuery('') }}><strong>{result.path}</strong><span>{result.snippet}</span></button>)}{!searching && !results.length && <small>No visible matches.</small>}</div>}
      ...
      <article className="note-panel panel">{!notePath ? <Empty icon={BookOpen} title="Choose a note" text="Select a note from the tree or search across your visible memory." /> : note.loading ? <Loading /> : note.error ? <ErrorState message={note.error} retry={note.reload} /> : note.data && <>
        ...
        <div className="markdown"><ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeSanitize]}>{obsidianMarkdown(note.data.markdown, selectedVault!)}</ReactMarkdown></div>
      </>}</article>
...
function AdminGateway() {
  ...
  const addRule = async () => { const subject_type = prompt('Subject type: user or group', 'group'); if (!subject_type) return; const subject = prompt('Subject name'); if (!subject) return; const tool_pattern = prompt('Tool pattern', '*. *'.replace(' ', '')); if (!tool_pattern) return; const effect = prompt('Effect: allow or deny', 'allow'); if (!effect) return; await api('/admin/permissions', { method: 'POST', body: JSON.stringify({ subject_type, subject, tool_pattern, effect }) }, csrf); await permissions.reload() }
  return <><div className="tabs"><button className={tab === 'servers' ? 'active' : ''} onClick={() => setTab('servers')}>servers</button><button className={tab === 'permissions' ? 'active' : ''} onClick={() => setTab('permissions')}>permission matrix</button></div>
    ...
    {tab === 'permissions' && <><div className="section-actions"><div><h2>Deny-wins permissions</h2><p>Explicit user and group denials are hard boundaries; defaults fill the gaps.</p></div><button className="button primary" onClick={addRule}><Plus />Add rule</button></div><div className="table-panel panel"><table><thead><tr><th>Subject</th><th>Tool pattern</th><th>Effect</th><th>Created</th><th /></tr></thead><tbody>{permissions.data?.permissions?.map((rule: any) => <tr key={rule.id}><td><Badge>{rule.subject_type}</Badge> #{rule.subject_id}</td><td><code>{rule.tool_pattern}</code></td><td><Badge tone={rule.effect === 'deny' ? 'danger' : 'safe'}>{rule.effect}</Badge></td><td>{formatDate(rule.created_at)}</td><td><button className="icon-button danger" onClick={async () => { await api(`/admin/permissions/${rule.id}`, { method: 'DELETE' }, csrf); await permissions.reload() }}><Trash2 /></button></td></tr>)}</tbody></table></div></>}
  </>
}

function AdminAudit() {
  const [tab, setTab] = useState<'tools' | 'commits'>('tools'); const calls = useLoad(() => api<any>('/audit/tools?limit=500'), []); const commits = useLoad(() => api<any>('/audit/commits?limit=500'), [])
